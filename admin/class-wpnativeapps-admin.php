...
class Wp_Native_Apps_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( $this->plugin_name, 'WPNativeApps',
		        array(
					...
					'WPNA_CONFIG_URL' => WPNA_CONFIG_URL,
		        )
		    );
	...
	*/
	public function wpna_get_settings(){
	    ...
	    if (file_exists(WPNA_CONFIG_PATH)) {
		...
		// 	}
		// }
		// echo file_get_contents(WPNA_CONFIG_PATH);die;
	...
	*/
	public function initializeConfig(){
		...
		file_put_contents(WPNA_CONFIG_PATH, stripslashes($configSettingJSON));
...
/*
* Hooked to action wpna_validateDomain hook
...
*/
function wpna_validateDomain(){
	$config = $this->wpna_get_settings();
...
function wpna_renderLiveChat(){ 
	ob_start();
	?>
	<div id="wpna_livechat-sticky" class="flex-row aic g10">
		<img src="<?php echo plugin_dir_url( __FILE__ ).'images/icon-help.png'?>">
		<div class="flex-column">
			<span><b>Hit a roadblock? </b><i>WPNativeApps</i> is brand new, so please <a href="https://wpnativeapps.com/support/" target="_blank">contact us</a>. We’re on hand to help!</span>
		</div>
	</div>
